...
def generate_regions_voronoi(height=768, width=1024, n_regions=6, return_type='img', seed=None):
    ...
    xs = np.random.randint(margin_x, width - margin_x, size=n_regions)
    ys = np.random.randint(margin_y, height - margin_y, size=n_regions)
    ...
    dists_sq = np.empty((height, width, n_regions), dtype=np.int64)
...
if __name__ == "__main__":
    H, W = 768, 1024
    n_regions = 10
    img = generate_regions_voronoi(H, W, n_regions=n_regions, seed=42)
