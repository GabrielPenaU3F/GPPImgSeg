...
rl_probs = RelaxationLabeler().label(ml_probs, neighborhood, n_iter=100, return_type='probs')
...
R_init = -200 * np.ones((3, 3)) + 400 * np.eye(3)
R_final = 100 * np.ones((3, 3)) + 100 * np.eye(3)
def adaptive_R(n):
    if n < 20:
        return R_init
    elif 20 <= n < 50:
...
hyper_labels = labeler_1.label(ml_probs, neighborhood, initial_total_balls=500,
                                  R=R_init, n_iter=100, return_type='img', input_type='probs')
hyper_img = align_labels(hyper_labels, ground_truth)

adaptive_labels = labeler_2.label(ml_probs, neighborhood, initial_total_balls=500,
